...
assert.match(app, /const speakContent=isAct\?'':`<h1>\$\{esc\(data\.title\)\}<\/h1>/, 'Part 2 should make the specific task its primary heading');
...
assert.match(app, /const speakContent=isAct\?'':`/, 'Part 3 must not evaluate the Speak-only facts template');
assert.match(app, /const actContent=!isAct\?'':`/, 'Part 2 must not evaluate the Act-only choices template');
