...
assert(rewardUi.includes('Write my takeaway'), 'celebration transitions into a written takeaway using direct program language');
assert(rewardUi.includes('<h2>Your takeaway</h2>'), 'written reflection has a clear purpose');
...
assert(rewardUi.includes('Putting it in your own words helps you remember it and use it next time.'), 'celebration explains the purpose of reflection');
assert(rewardUi.includes('Save and return to Learning Journey'), 'written reflection returns to the canonical course map');
assert(rewardUi.includes('Skip for now'), 'skipping the reflection still allows progress');
assert(rewardUi.includes('Write one or two sentences.'), 'reflection requests a concise written takeaway');
assert(rewardUi.includes('p.utl-reflection-guidance') && rewardUi.includes('font:italic 400 12px'), 'reflection guidance is visually subordinate and aligned with the prompt');
assert(rewardUi.includes('reflectionNote.value.trim().length < 10'), 'brief accidental entries do not enable saving');
assert(!rewardUi.includes('data-reflection-choice'), 'preset takeaway choices no longer replace learner reflection');
