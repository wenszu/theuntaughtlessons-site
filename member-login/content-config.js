...
(function () {
  ...
  function engagementAnalyticsHref() {
    if (inPhasePracticeRoot()) return "../../../assets/engagement-analytics.js?v=20260925-mobile-v1";
    return "../assets/engagement-analytics.js?v=20260925-mobile-v1";
  }
  ...
  function sanitizeMediaUrl(url) {
    ...
    if (vimeoPlayerMatch) return addVimeoPlaybackParams("https://player.vimeo.com/video/" + vimeoPlayerMatch[1] + (vimeoPlayerMatch[2] || ""));
    var vimeoMatch = value.match(/vimeo\.com\/(?:video\/)?(\d+)(?:\/([a-zA-Z0-9]+))?/);
    if (vimeoMatch) {
      return addVimeoPlaybackParams("https://player.vimeo.com/video/" + vimeoMatch[1] + (vimeoMatch[2] ? "?h=" + encodeURIComponent(vimeoMatch[2]) : ""));
    }
  ...
  function directMediaUrl(url) {
    ...
    if (!value) return "";
    var vimeoPlayerMatch = value.match(/player\.vimeo\.com\/video\/(\d+)/);
    if (vimeoPlayerMatch) return "https://vimeo.com/" + vimeoPlayerMatch[1];
    var vimeoMatch = value.match(/vimeo\.com\/(?:video\/)?(\d+)(?:\/([a-zA-Z0-9]+))?/);
  ...
  function renderIframe(url, title, analyticsId) {
    ...
    var mobileLaunch = directUrl
      ? isVimeo
        ? '<div class="ws-mobile-video-launch"><div><strong>Video pausing on your phone?</strong><span>Open the video directly in Vimeo and continue from the same point.</span></div><a href="' + escapeHtml(directUrl) + '" target="_blank" rel="noopener">Open on Vimeo &rarr;</a></div>'
    ...
    var analyticsAttrs = isVimeo
      ...
      : '';
    var recovery = isVimeo
      ? '<div class="ws-vimeo-recovery" hidden role="status" aria-live="polite"><strong>Playback is taking longer than expected.</strong><span data-vimeo-recovery-copy>Retry from where you stopped, or open the video directly.</span><div><button type="button" data-vimeo-retry>Retry video</button>' + (directUrl ? '<a href="' + escapeHtml(directUrl) + '" target="_blank" rel="noopener">Open on Vimeo</a>' : '') + '</div></div>'
      : '';
    return mobileLaunch + '<div class="ws-media-frame"><iframe src="' + escapeHtml(src) + '" title="' + escapeHtml(title || "Video player") + '"' + analyticsAttrs + ' loading="eager" allow="autoplay; fullscreen; picture-in-picture; clipboard-write; encrypted-media; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe><button type="button" class="ws-media-expand" aria-label="View in full screen"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M8 3H5a2 2 0 0 0-2 2v3"/><path d="M21 8V5a2 2 0 0 0-2-2h-3"/><path d="M3 16v3a2 2 0 0 0 2 2h3"/><path d="M16 21h3a2 2 0 0 0 2-2v-3"/></svg></button>' + recovery + '</div>';
  }

  function videoAccessHelp(url) {
    if (/vimeo\.com\//.test(String(url || ""))) {
      return '<details class="ws-video-access-help ws-vimeo-help"><summary aria-label="Show or hide Vimeo troubleshooting"><span>Video pausing or freezing?</span><span class="ws-video-help-actions"><small>Try these fixes</small><span class="ws-video-help-toggle" aria-hidden="true"></span></span></summary><div class="ws-access-help-body">' +
        '<div class="ws-access-guide">' +
          '<section><h4>Quick recovery</h4><ol><li><strong>Use Retry video:</strong> The player will return to your last position.</li><li><strong>Lower the quality:</strong> In the player settings, choose Auto or a lower resolution.</li><li><strong>Refresh once:</strong> Reload the lesson if the player remains unresponsive.</li></ol></section>' +
          '<section><h4>Connection checks</h4><ol><li><strong>Try another network:</strong> Switch briefly to a mobile hotspot to identify an office-network restriction.</li><li><strong>Pause other downloads:</strong> Video quality adjusts to available bandwidth.</li><li><strong>Update the browser:</strong> Current Chrome, Edge, Safari and Firefox are supported.</li></ol></section>' +
          '<section><h4>Corporate networks</h4><ol><li><strong>Ask IT to allow Vimeo:</strong> Permit <code>*.vimeo.com</code>, <code>*.vimeocdn.com</code>, <code>*.akamaized.net</code> and <code>*.cloudfront.net</code>.</li><li><strong>Allow web traffic:</strong> Ports 80 and 443 must be open.</li><li><strong>Check managed blockers:</strong> Security extensions can interrupt Vimeo playback and player messages.</li></ol></section>' +
        '</div>' +
      '</div></details>';
    }
    var email = currentUser().email || "";
    ...
    return '<details class="ws-video-access-help"><summary aria-label="Show or hide video troubleshooting"><span>Video not opening?</span><span class="ws-video-help-actions"><small>Try these fixes</small><span class="ws-video-help-toggle" aria-hidden="true"></span></span></summary><div class="ws-access-help-body">' +
      ...
      '<div class="ws-access-guide">' +
        '<section><h4>Quick checks</h4><ol>' +
          ...
          '<li><strong>Confirm your membership email:</strong> Make sure the email shown here matches the address used for your membership.</li>' +
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      ".ws-links{display:flex;align-items:center;justify-content:center;gap:16px;min-width:0}.ws-link{position:relative;min-height:54px;display:inline-flex;align-items:center;border-bottom:3px solid transparent;color:rgba(255,255,255,.72);font:700 13px Lato,sans-serif;letter-spacing:0;text-decoration:none;white-space:nowrap;transition:color .3s ease-in-out}.ws-link.ws-active{color:var(--ws-white);border-bottom-color:var(--ws-gold)}.ws-link:not(.ws-active)::after{content:'';position:absolute;left:50%;bottom:0;width:0;height:3px;border-radius:2px;background-color:var(--ws-gold);transition:left .3s ease-in-out,width .3s ease-in-out}.ws-link:not(.ws-active):hover{color:var(--ws-white)}.ws-link:not(.ws-active):hover::after{left:0;width:100%}.ws-sep{color:rgba(255,255,255,.32);font-family:Lato,sans-serif}",
      ...
      ".ws-media-frame{position:relative;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:8px;background:#101820}.ws-media-frame iframe{position:absolute;inset:0;width:100%;height:100%;border:0}.ws-media-expand{position:absolute;top:10px;right:10px;z-index:4;width:44px;height:44px;padding:0;border:1px solid rgba(255,255,255,.35);border-radius:6px;background:rgba(0,20,40,.55);color:#fff;display:grid;place-items:center;cursor:pointer}.ws-media-expand svg{width:18px;height:18px}.ws-media-expand:hover{background:rgba(0,20,40,.82)}.ws-media-frame.ws-media-expanded{position:fixed;inset:0;z-index:9999;width:100vw;height:100vh;height:100dvh;padding-bottom:0;border-radius:0;background:#000}.ws-media-frame.ws-media-expanded iframe{position:absolute;inset:0;width:100%;height:100%}body.ws-media-lock{overflow:hidden}.ws-mobile-video-launch{display:none}.ws-vimeo-recovery{position:absolute;left:16px;right:16px;bottom:16px;z-index:3;display:grid;grid-template-columns:1fr auto;gap:3px 16px;align-items:center;padding:13px 14px;border:1px solid rgba(255,255,255,.25);border-radius:8px;background:rgba(0,31,61,.96);box-shadow:0 8px 28px rgba(0,0,0,.3);color:#fff;font:14px/1.35 Lato,Arial,sans-serif}.ws-vimeo-recovery[hidden]{display:none}.ws-vimeo-recovery strong{grid-column:1;color:#fff;font-size:15px}.ws-vimeo-recovery span{grid-column:1;color:#dce7f2}.ws-vimeo-recovery>div{grid-column:2;grid-row:1/3;display:flex;gap:8px}.ws-vimeo-recovery button,.ws-vimeo-recovery a{display:inline-flex;align-items:center;justify-content:center;min-height:38px;padding:8px 12px;border:1px solid #fff;border-radius:6px;background:#fff;color:var(--ws-navy);font:700 12px Lato,Arial,sans-serif;text-decoration:none;cursor:pointer}.ws-vimeo-recovery a{background:transparent;color:#fff}.ws-vimeo-help code{font-size:11px}",
  ...
  function hydrateOrganizationAccess() {
    var sections = qsa("[data-organization-access]");
    if (!sections.length) return;
    var cacheKey = "utl_organization_access_v1";
    var cached = null;
    try {
    ...
    function reveal(data) {
      var organizations = data && Array.isArray(data.organizations) ? data.organizations : [];
      if (!data || data.hasAccess !== true || !organizations.length) return;
      var label = organizations.length === 1
        ? organizations[0].name + " · " + organizations[0].roleLabel
        : organizations.length + " organizations";
      sections.forEach(function (section) {
        section.hidden = false;
        var target = section.querySelector("[data-organization-access-label]");
    ...
    import(firebaseHref()).then(function (firebaseAuth) {
      return firebaseAuth.getMyOrganizationAccess();
    }).then(function (data) {
      try {
      ...
      // Organization access is optional. Leave the entry hidden when unavailable.
    });
  }
  ...
  function scopeLearnerDeviceState(email) {
    var normalizedEmail = String(email || "").trim().toLowerCase();
    if (!normalizedEmail) return;
    var previousScope = String(localStorage.getItem(ACCOUNT_SCOPE_KEY) || "").trim().toLowerCase();
    var cachedUser = String(localStorage.getItem(USER_KEY) || "").trim().toLowerCase();
    var previousEmail = previousScope || (cachedUser && cachedUser !== "member" ? cachedUser : "");
    ...
    localStorage.setItem(ACCOUNT_SCOPE_KEY, normalizedEmail);
  }
  ...
  function accountSignInMethods(account) {
    var labels = {
      "google.com": "Google",
    ...
    return (account.signInProviderIds || []).map(function (providerId) {
      return labels[providerId] || "Connected account";
  ...
  function accountPageHtml(account) {
    ...
    var connectedMethodsHtml = connectedMethods.length
      ? connectedMethods.map(function (label) { return '<span class="ws-account-method"><span class="ws-account-method-check" aria-hidden="true">✓</span>' + escapeHtml(label) + '</span>'; }).join("")
      : '<p>No connected provider is recorded for this account.</p>';
    var accessHtml = '<section class="ws-card ws-account-card"><div class="ws-section-head"><div><span class="ws-kicker">Access</span><h2>Account and sign-in</h2><p>Use the same email address each time so your work stays with this account.</p></div></div><div class="ws-account-access"><div class="ws-account-access-panel"><h3>Login email</h3><span class="ws-account-access-value">' + escapeHtml(account.email) + '</span><p>This is the email registered with your membership. It cannot be changed here.</p></div><div class="ws-account-access-panel"><h3>Ways you can sign in</h3><div class="ws-account-methods"><span class="ws-account-method"><span class="ws-account-method-check" aria-hidden="true">✓</span>Email sign-in link</span>' + connectedMethodsHtml + '</div><p>An email sign-in link is available as a backup for this registered address.</p><p class="ws-account-access-note">Contact the program team if your login email needs to change. Your role, cohort and program access are also managed by the program team.</p></div></div></section>';
  ...
  function homePageHtml() {
    ...
    var certSection = allDone ? '<section class="ws-section"><div class="ws-section-head"><h2>Your credential</h2></div><article class="ws-phase-card"><div class="ws-phase-stripe" style="background:#2C7A4B"></div><div class="ws-phase-number" style="color:rgba(44,122,75,.15)">&#10003;</div><div class="ws-phase-content"><span class="ws-kicker">Complete</span><h2>Certificate and verified credential</h2><p>Your downloadable certificate and public verification link are ready. Add the credential to LinkedIn or share it through your preferred app.</p></div><div class="ws-phase-actions"><span class="ws-pill ws-pill-green">Done</span><a class="ws-button" href="../certificate/index.html" target="_blank" rel="noopener">Open certificate &amp; sharing <span aria-hidden="true">↗</span></a></div></article></section>' : "";
  ...
  function assessmentJourneyNudgeHtml(model, next, progress) {
    ...
    var action = inProgress ? "Resume diagnostic" : checkpoint ? "Take checkpoint · about 30 min" : "Take diagnostic · about 30 min";
  ...
  function journeyActivityRowHtml(activity, next, phaseIsUnlocked, activityIsUnlocked, sequenceLabel) {
    ...
    var reviewHref = activity.kind === "Exercise"
      ? activity.href + (activity.href.indexOf("?") === -1 ? "?" : "&") + "review=latest"
      : activity.href;
    var action = state === "done"
      ? '<a href="' + escapeHtml(reviewHref) + '">Review</a>'
      : state === "progress"
    ...
    var availableHref = state === "done" ? reviewHref : activity.href;
    var availableAction = '<a class="ws-button" href="' + escapeHtml(availableHref) + '">' + (state === "done" ? "Review " : state === "progress" ? "Resume " : "Start ") + escapeHtml(sequenceLabel) + ' &rarr;</a>';
  ...
  function assessmentsSection() {
    ...
    var diagnostic = assessmentJourneyStatus("diagnostic");
    var checkpoint = assessmentJourneyStatus("checkpoint");
    var diagnosticCard = diagnostic.complete
      ? '<article class="ws-phase-card"><div class="ws-phase-stripe" style="background:#2C7A4B"></div><div class="ws-phase-number">A</div><div class="ws-phase-content"><span class="ws-kicker">Before the program</span><h2>The diagnostic</h2><p>Your starting Think, Speak, and Act snapshot is saved. Review your score and the details behind it any time.</p></div><div class="ws-phase-actions"><span class="ws-pill ws-pill-green">Done</span><a class="ws-button" href="' + resultsHref + '">Review my diagnostic &rarr;</a></div></article>'
      : '<article class="ws-phase-card"><div class="ws-phase-stripe"></div><div class="ws-phase-number">A</div><div class="ws-phase-content"><span class="ws-kicker">Before the program</span><h2>The diagnostic</h2><p>Take this once before you begin. It measures your starting Think, Speak, and Act skills. About 30 minutes.</p></div><div class="ws-phase-actions"><a class="ws-button" href="' + diagnosticHref + '">Open diagnostic &rarr;</a></div></article>';
    var checkpointCard = checkpoint.complete
      ? '<article class="ws-phase-card"><div class="ws-phase-stripe" style="background:#2C7A4B"></div><div class="ws-phase-number">B</div><div class="ws-phase-content"><span class="ws-kicker">After the program</span><h2>The checkpoint</h2><p>Your post-program snapshot is saved. Review your score, the details behind it, and the before-and-after comparison any time.</p></div><div class="ws-phase-actions"><span class="ws-pill ws-pill-green">Done</span><a class="ws-button" href="' + resultsHref + '">Review my checkpoint &rarr;</a></div></article>'
      : phase3Done ? '<article class="ws-phase-card"><div class="ws-phase-stripe"></div><div class="ws-phase-number">B</div><div class="ws-phase-content"><span class="ws-kicker">After the program</span><h2>The checkpoint</h2><p>Repeat the same Think, Speak, and Act skills with a different scenario set. Your checkpoint result is saved separately for a before-and-after comparison. About 30 minutes.</p></div><div class="ws-phase-actions"><a class="ws-button" href="' + checkpointHref + '">Open checkpoint &rarr;</a></div></article>' : '<article class="ws-phase-card ws-locked"><div class="ws-phase-stripe"></div><div class="ws-phase-number">B</div><div class="ws-phase-content"><span class="ws-kicker">After the program</span><h2>The checkpoint</h2><p>Complete all three phases to unlock the post-program comparison. It uses the same assessment and scoring with a different scenario set. About 30 minutes.</p></div><div class="ws-phase-actions"><span class="ws-pill ws-pill-locked">Locked</span><span class="ws-button ws-disabled">Open checkpoint &rarr;</span></div></article>';
    return '<section class="ws-section" id="assessments"><div class="ws-section-head"><h2>Assessments</h2><span class="ws-count">Before and after</span></div><div class="ws-phase-list">' + diagnosticCard + checkpointCard + '</div></section>';
  }
  ...
  function showPhaseCompletionModal(phaseKey, certificateEnabled) {
    ...
    var certLink = allDone && certificateEnabled !== false
      ? '<a class="ws-button" href="../certificate/index.html" target="_blank" rel="noopener" style="margin-top:8px">View your certificate <span aria-hidden="true">↗</span></a>'
      : "";
  ...
  function setVimeoRecovery(frame, visible, message) {
    ...
    if (!recovery) return;
    var copy = recovery.querySelector("[data-vimeo-recovery-copy]");
  ...
  function bindVimeoFrame(frame) {
    ...
    loadVimeoSdk().then(function (Vimeo) {
      ...
      player.on("play", function () { playing = true; lastEventAt = Date.now(); setVimeoRecovery(frame, false); watchForStall(); dispatchVimeoProgress(frame, { eventName: "play" }); });
      player.on("timeupdate", function (data) {
        if (playing) watchForStall();
        setVimeoRecovery(frame, false);
        var now = Date.now();
      ...
      if (retry) retry.addEventListener("click", function () {
        retry.disabled = true;
        setVimeoRecovery(frame, false);
        Promise.resolve(lastPosition > 0 ? player.setCurrentTime(lastPosition) : null)
          .then(function () { return player.play(); })
          .catch(function () { setVimeoRecovery(frame, true, "The embedded player is still unavailable. Open the video directly or try another network."); })
  ...
  // Some environments (locked-down corporate browser policies, certain webviews) reject a
  // fullscreen request made from inside the cross-origin Vimeo iframe, so the player's own
  // fullscreen button appears to expand and then immediately reverts. This button instead
  // requests fullscreen on the same-origin page itself, which is allowed in more places, and
  // falls back to a viewport-covering CSS overlay when the Fullscreen API is unavailable or refused.
  function mediaFrameIsExpanded(frameWrap) {
    return frameWrap.classList.contains("ws-media-expanded") || document.fullscreenElement === frameWrap || document.webkitFullscreenElement === frameWrap;
  }
  function syncMediaExpandButton(frameWrap) {
    var expanded = mediaFrameIsExpanded(frameWrap);
    document.body.classList.toggle("ws-media-lock", expanded);
    var btn = frameWrap.querySelector(".ws-media-expand");
    if (btn) btn.setAttribute("aria-label", expanded ? "Exit full screen" : "View in full screen");
  }
  function setMediaFallbackExpanded(frameWrap, expanded) {
    frameWrap.classList.toggle("ws-media-expanded", expanded);
    syncMediaExpandButton(frameWrap);
  }
  function toggleMediaExpand(frameWrap) {
    if (mediaFrameIsExpanded(frameWrap)) {
      if (document.fullscreenElement === frameWrap && document.exitFullscreen) document.exitFullscreen().catch(function () {});
      else if (document.webkitFullscreenElement === frameWrap && document.webkitExitFullscreen) document.webkitExitFullscreen();
      else setMediaFallbackExpanded(frameWrap, false);
      return;
    }
    var request = frameWrap.requestFullscreen || frameWrap.webkitRequestFullscreen;
    if (request) Promise.resolve(request.call(frameWrap)).catch(function () { setMediaFallbackExpanded(frameWrap, true); });
    else setMediaFallbackExpanded(frameWrap, true);
  }
  document.addEventListener("click", function (event) {
    var btn = event.target.closest && event.target.closest(".ws-media-expand");
    if (!btn) return;
    var frameWrap = btn.closest(".ws-media-frame");
    if (frameWrap) toggleMediaExpand(frameWrap);
  });
  ["fullscreenchange", "webkitfullscreenchange"].forEach(function (eventName) {
    document.addEventListener(eventName, function () { qsa(".ws-media-frame").forEach(syncMediaExpandButton); });
  });
  document.addEventListener("keydown", function (event) {
    if (event.key !== "Escape") return;
    qsa(".ws-media-frame.ws-media-expanded").forEach(function (frameWrap) { setMediaFallbackExpanded(frameWrap, false); });
  });
