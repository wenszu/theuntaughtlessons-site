...
(function () {
  ...
  function videoAccessHelp() {
    var email = currentUser().email || "";
    var emailText = email.indexOf("@") > -1 ? ' Your workspace email is <strong>' + escapeHtml(email) + '</strong>.' : "";
    return '<details class="ws-video-access-help"><summary><span>Video not opening?</span><small>Fix Google Group access once</small></summary><div class="ws-access-help-body"><p class="ws-access-warning"><strong>Do not request access to each video.</strong> The videos unlock when your Google account is in the UTL Members group.</p><ol><li><strong>Use the right Google account:</strong> Make sure you are signed into Google with the same email you use for this workspace.' + emailText + '</li><li><strong>Open the member group:</strong> Go to <a href="https://groups.google.com/g/utl-members" target="_blank" rel="noopener">UTL Members on Google Groups</a>.</li><li><strong>Join or ask to be added:</strong> If you see <strong>Join group</strong>, join the group. If Google says you need an invitation, reply to your welcome email and ask to be added to <strong>utl-members@googlegroups.com</strong>.</li><li><strong>Refresh the workspace:</strong> Return here and refresh this page. The video should load without asking for per-video permission.</li></ol><div class="ws-access-shot-row" aria-label="Video access visual guide"><div class="ws-access-shot"><strong>1. Check account</strong><span>Use the same Google email as your member login.</span></div><div class="ws-access-shot"><strong>2. Join group</strong><span>Open the UTL Members group and join or request help.</span></div><div class="ws-access-shot"><strong>3. Refresh video</strong><span>Come back to the workspace and reload the page.</span></div></div></div></details>';
  }

  function renderEmbeddedMedia(url, title) {
    var frame = renderIframe(url, title);
    return frame ? frame + videoAccessHelp() : "";
  }
