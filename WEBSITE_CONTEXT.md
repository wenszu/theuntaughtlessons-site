...
- Cache-busting `?v=...` query strings use one shared release value across the repo. Do not hand-edit individual `?v=` values. Run `node scripts/sync-cache-versions.js --version=<release>` only when intentionally advancing the shared key, and `node scripts/sync-cache-versions.js --check` to verify consistency. Cloudflare `_headers` sets `Cache-Control: public, max-age=0, must-revalidate`, so browsers verify cached pages and modules before reuse. This prevents an old Firebase module from silently outliving a new caller. The GitHub `Verify deployment cache` workflow checks the shared key and runs the full suite without rewriting files, pushing a bot commit, or triggering a second deployment. Tests that assert cache-busting use a pattern such as `/content-config\.js\?v=[\w-]+/` rather than a literal release value.
