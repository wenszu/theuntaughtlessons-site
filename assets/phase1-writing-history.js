(function () {
  ...
  const style=document.createElement('style');style.textContent='.phase1-work-state{display:flex;align-items:center;justify-content:space-between;gap:16px;margin:0 0 12px;padding:13px 14px;border:1px solid var(--line,#c7d8e8);border-radius:8px;background:var(--paper,#f7f4ef)}.phase1-work-state strong,.phase1-work-state span{display:block}.phase1-work-state strong{color:var(--navy,#003366)}.phase1-work-state span{margin-top:3px;color:var(--steel,#4d7094);font-size:12px}.phase1-work-actions{display:flex;align-items:center;flex-wrap:wrap;justify-content:flex-end;gap:8px}.phase1-work-actions select,.phase1-work-actions button{min-height:40px;border:1px solid var(--navy,#003366);border-radius:7px;background:#fff;color:var(--navy,#003366);padding:8px 12px;font-weight:700}.phase1-work-actions select{border-color:var(--line,#c7d8e8);background:#fff}.phase1-work-actions [data-resume]{background:var(--navy,#003366);color:#fff}.phase1-work-actions [data-latest],.phase1-work-actions [data-edit]{background:#fff;color:var(--navy,#003366)}@media(min-width:981px){.layout>aside{position:sticky;top:96px;max-height:calc(100dvh - 112px);overflow-y:auto;overscroll-behavior:contain;padding-right:4px}.layout>aside>.reference-panel{position:static}.layout>aside>.reference-panel .panel-body{max-height:none;overflow:visible}}@media(max-width:900px){.phase1-work-state{display:block}.phase1-work-actions{justify-content:flex-start;margin-top:10px}.phase1-work-actions select{flex:1 1 100%;width:100%}}@media(max-width:560px){.phase1-work-actions button{width:100%}}';document.head.appendChild(style);
  let saveTimer = 0;
  let remoteAttempts = [];
  ...
  function render() {
    ...
    resumeBtn.hidden=!hasDraft(draft)||state==='draft';latestBtn.hidden=!latest||state==='viewing';
    select.hidden=saved.length<2;select.innerHTML='<option value="">Choose a previous submission</option>'+saved.slice().reverse().map((item,index)=>`<option value="${attemptKey(item)}">Submission ${saved.length-index} · ${formatDate(item.timestamp)}</option>`).join('');if(viewingAttemptKey)select.value=viewingAttemptKey;
  }
  const panel=document.createElement('section');panel.className='phase1-work-state';panel.innerHTML='<div><strong></strong><span></span></div><div class="phase1-work-actions"><select aria-label="Choose a previous submission"></select><button type="button" data-resume>Continue unfinished draft</button><button type="button" data-latest>View latest submission</button><button type="button" data-edit hidden>Use this response for a new attempt</button></div>';
  modeSwitch.parentNode.insertBefore(panel,modeSwitch);
  const titleEl=panel.querySelector('strong'),copyEl=panel.querySelector('span'),select=panel.querySelector('select'),resumeBtn=panel.querySelector('[data-resume]'),latestBtn=panel.querySelector('[data-latest]'),editBtn=panel.querySelector('[data-edit]');
  ...
  latestBtn.addEventListener('click',()=>{const list=attempts();const latest=list[list.length-1];if(latest){viewingAttemptKey=attemptKey(latest);applyResponse(latest.userResponse,true);}});
