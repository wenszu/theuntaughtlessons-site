...
Best, Yutee Elle`;
  ...
  function notesHtml() {
    if (state.notesMode === 'open') return `<textarea class="aiko-textarea" id="openNotes" placeholder="Write your ${TARGET_SECONDS}-second talking points here...">${escapeHtml(state.openNotes)}</textarea>`;
  ...
  function renderPreparation() {
    shell(`<section class="aiko-panel"><div class="aiko-panel-head"><p class="aiko-progress">Step 2 · Preparation notes</p><h2>Prepare your ${TARGET_SECONDS}-second talk.</h2><p>Use the email as your source. Prepare spoken points that sound natural and fit roughly ${TARGET_WORDS}.</p></div><div class="aiko-step"><div class="aiko-prep-grid"><section class="aiko-source"><h3>Email to Aiko</h3><p class="aiko-email">${escapeHtml(EMAIL_TO_AIKO)}</p></section><section class="aiko-notes"><h3>Talking points prep</h3><div class="aiko-mode" aria-label="Preparation format"><button class="${state.notesMode === 'open' ? 'is-active' : ''}" data-mode="open">Open response</button><button class="${state.notesMode === 'sections' ? 'is-active' : ''}" data-mode="sections">Three-section response</button></div><p class="aiko-guidance"><strong>Choose one format:</strong> Open response gives you a blank page. Three-section response guides you through the bottom line, three reasons, and close.</p><div id="notesArea">${notesHtml()}</div></section></div>${actions('recordButton', 'I am ready to record', 'briefBack')}</div></section>`);
    ...
    document.getElementById('briefBack').addEventListener('click', renderBrief);
  ...
  async function saveResult() {
    ...
    window.UTLRewardEvents?.awardCompletionExercise({ appId: APP_ID, title: mode === '60' ? 'Explain to Aiko in 60 seconds complete' : 'Explain to Aiko complete', body: `Your ${TARGET_SECONDS}-second explanation was saved.` });
    button.textContent = 'Saved'; status.textContent = 'Saved. This exercise is complete.';
    const complete = document.createElement('div'); complete.className = 'aiko-complete'; complete.innerHTML = `<h3>Well done.</h3><p>You turned the email into a spoken explanation and practiced making the logic easy to follow.</p><div class="aiko-actions"><a class="aiko-link" href="../../member-login/index.html#phase2">Back to workspace</a></div>`; status.after(complete);
  }
