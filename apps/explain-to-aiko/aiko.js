...
Best, Yutee Elle`;
  ...
  function renderPreparation() {
    shell(`<section class="aiko-panel"><div class="aiko-panel-head"><p class="aiko-progress">Step 2 · Preparation notes</p><h2>Prepare your ${TARGET_SECONDS}-second talk.</h2><p>Use the email as your source. Prepare spoken points that sound natural and fit roughly ${TARGET_WORDS}.</p></div><div class="aiko-step"><div class="aiko-prep-grid"><section class="aiko-source"><h3>Email to Aiko</h3><p class="aiko-email">${escapeHtml(EMAIL_TO_AIKO)}</p></section><section class="aiko-notes"><h3>Your response</h3><div class="aiko-mode" aria-label="Preparation format"><button class="${state.notesMode === 'open' ? 'is-active' : ''}" data-mode="open">Open response</button><button class="${state.notesMode === 'sections' ? 'is-active' : ''}" data-mode="sections">Three-section response</button></div><p class="aiko-guidance"><strong>Choose one format:</strong> Use <strong>Open response</strong> if you want to write in your own free-form structure; this is the more difficult option. Use <strong>Three-section response</strong> if you want the easier guided option. In both formats, use BSP and the Rule of three.</p><div id="notesArea">${notesHtml()}</div></section></div>${actions('recordButton', 'I am ready to record', 'briefBack')}</div></section>`);
